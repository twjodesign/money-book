...
export default function RecurringPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/recurring?accountId=${currentAccount.id}`).then((r) => r.json()).then(setItems);
    fetch(`/api/categories?type=${currentAccount.type}`).then((r) => r.json()).then(setCategories);
  }, [currentAccount]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    await fetch("/api/recurring", {
      ...
      body: JSON.stringify({ account_id: currentAccount.id, category_id: categoryId, direction, title, amount: parseFloat(amount), note, day_of_month: parseInt(dayOfMonth) }),
    });
  ...
  async function handleGenerate() {
    ...
    setShowSuccess(data.generated > 0 ? `已產生 ${data.generated} 筆本月固定收支` : "本月已全部入帳");
    ...
    <div className="space-y-4 animate-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-1">設定一次，每月自動入帳</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card text-center">
          <p className="section-label mb-1">固定收入</p>
          <p className="text-lg font-bold text-[var(--green)]">{fmt(totalIncome)}</p>
        </div>
        <div className="card text-center">
          <p className="section-label mb-1">固定支出</p>
          <p className="text-lg font-bold text-[var(--red)]">{fmt(totalExpense)}</p>
        </div>
        <div className="card text-center">
          <p className="section-label mb-1">每月淨額</p>
      ...
      {showForm && (
        <div className="card animate-in">
          <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-3">新增固定收支</h3>
          <form onSubmit={handleSave} className="space-y-4">
            <div className="flex gap-2 bg-[var(--bg)] p-1 rounded-2xl">
              <button type="button"
                ...
                onClick={() => setDirection("income")}>固定收入</button>
              <button type="button"
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  direction === "expense" ? "bg-[var(--red)] text-white shadow-sm" : "text-[var(--text-muted)]"
                }`}
                onClick={() => setDirection("expense")}>固定支出</button>
            </div>
            ...
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="薪水、Netflix 訂閱..." required />
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <label className="block section-label mb-1.5">每月幾號</label>
      ...
      {incomeItems.length > 0 && (
        <div>
          <p className="section-label px-1 mb-2">固定收入</p>
          <div className="space-y-2">
            {incomeItems.map((item) => (
              <div key={item.id} className="card !p-3 !rounded-2xl flex items-center justify-between group">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[var(--green-light)] flex items-center justify-center text-lg">{item.category_icon}</div>
                  <div>
                    <p className="text-sm font-semibold">{item.title || item.category_name}</p>
                ...
                <div className="flex items-center gap-2">
                  <span className="tag tag-green">+{fmt(item.amount)}</span>
                  <button onClick={() => handleDelete(item.id)} className="opacity-0 group-hover:opacity-100 text-xs p-1 hover:text-[var(--red)] transition-opacity">🗑️</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {expenseItems.length > 0 && (
        <div>
          <p className="section-label px-1 mb-2">固定支出</p>
          <div className="space-y-2">
            {expenseItems.map((item) => (
              <div key={item.id} className="card !p-3 !rounded-2xl flex items-center justify-between group">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[var(--red-light)] flex items-center justify-center text-lg">{item.category_icon}</div>
                  <div>
                    <p className="text-sm font-semibold">{item.title || item.category_name}</p>
                    <p className="text-[10px] text-[var(--text-muted)]">
                      {item.title ? item.category_name + " · " : ""}每月 {item.day_of_month} 號{item.note && ` · ${item.note}`}
                ...
                <div className="flex items-center gap-2">
                  <span className="tag tag-red">-{fmt(item.amount)}</span>
                  <button onClick={() => handleDelete(item.id)} className="opacity-0 group-hover:opacity-100 text-xs p-1 hover:text-[var(--red)] transition-opacity">🗑️</button>
                </div>
              </div>
            ))}
