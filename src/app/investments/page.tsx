...
export default function InvestmentsPage() {
  ...
  const [hTotalCost, setHTotalCost] = useState("");
  ...
  async function lookupSymbol(symbol: string) {
    ...
    setHLookingUp(true);
    try {
      const res = await fetch(`/api/stock-lookup?symbol=${encodeURIComponent(symbol)}&type=${getHoldingAcctType()}`);
      const data = await res.json();
      if (data.name) setHName(data.name);
    } finally { setHLookingUp(false); }
  }

  function handleTotalCostChange(val: string) {
    setHTotalCost(val);
    const total = parseFloat(val), price = parseFloat(hAvgCost);
    if (!isNaN(total) && !isNaN(price) && price > 0) setHQuantity((total / price).toFixed(4).replace(/\.?0+$/, ""));
  }

  function handleAvgCostChange(val: string) {
    setHAvgCost(val);
    const total = parseFloat(hTotalCost), price = parseFloat(val);
    if (!isNaN(total) && !isNaN(price) && price > 0) setHQuantity((total / price).toFixed(4).replace(/\.?0+$/, ""));
  }

  function handleQuantityChange(val: string) {
    setHQuantity(val);
    const qty = parseFloat(val), price = parseFloat(hAvgCost);
    if (!isNaN(qty) && !isNaN(price) && price > 0) setHTotalCost((qty * price).toFixed(0));
  }
  ...
  async function handleSaveHolding(e: React.FormEvent) {
    ...
    await fetch("/api/holdings", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ invest_account_id: addHoldingFor, symbol: hSymbol.toUpperCase(), name: hName, quantity: parseFloat(hQuantity), avg_cost: parseFloat(hAvgCost), current_price: parseFloat(hCurrentPrice), purchase_date: hPurchaseDate || null }),
    });
  ...
  return (
    <div className="space-y-4 animate-in">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">投資帳戶</h2>
          <p className="text-sm text-[var(--text-muted)] mt-1">追蹤股票與加密貨幣</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card text-center">
          <p className="section-label mb-1">總成本</p>
          <p className="text-lg font-bold">{fmt(grandCost)}</p>
        </div>
        <div className="card text-center">
          <p className="section-label mb-1">總市值</p>
          <p className="text-lg font-bold text-[var(--blue)]">{fmt(grandMarket)}</p>
        </div>
        <div className="card text-center">
          <p className="section-label mb-1">總報酬</p>
      ...
      {showAccountForm && (
        <div className="card animate-in">
          <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-3">新增投資帳戶</h3>
          <form onSubmit={handleSaveAccount} className="space-y-4">
            <div>
              <label className="block section-label mb-2">類型</label>
                  ...
                  return (
                    <button key={t} type="button"
                      className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                        acctType === t ? "bg-[var(--accent)] text-white shadow-sm" : "text-[var(--text-muted)]"
                      }`}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block section-label mb-1.5">券商/交易所</label>
          ...
          <div key={acct.id} className="card">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => handleDeleteAccount(acct.id)} className="text-xs p-1 hover:text-[var(--red)] transition-opacity text-[var(--text-muted)]">🗑️</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center mb-3">
              <div className="bg-[var(--bg)] rounded-xl p-2">
                <p className="section-label mb-0.5">成本</p>
                <p className="text-xs font-bold">{fmt(acctCost)}</p>
              </div>
              <div className="bg-[var(--bg)] rounded-xl p-2">
                <p className="section-label mb-0.5">市值</p>
                <p className="text-xs font-bold text-[var(--blue)]">{fmt(acctMarket)}</p>
              </div>
              <div className="bg-[var(--bg)] rounded-xl p-2">
                <p className="section-label mb-0.5">報酬</p>
            ...
            {addHoldingFor === acct.id && (
              <div className="bg-[var(--bg)] rounded-xl p-3 mb-3 animate-in">
                <h4 className="section-label mb-3">新增持股</h4>
                <form onSubmit={handleSaveHolding} className="space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input type="text" value={hSymbol} onChange={(e) => setHSymbol(e.target.value)}
                        onBlur={(e) => lookupSymbol(e.target.value)}
                        placeholder={acct.type === "tw_stock" ? "2330" : acct.type === "us_stock" ? "NVDA" : "BTC"}
                        required className="!text-xs !py-2 uppercase" />
                    </div>
                    <div>
                      <label className="block section-label mb-1">股票名稱</label>
                      <input type="text" value={hName} onChange={(e) => setHName(e.target.value)}
                        placeholder={acct.type === "tw_stock" ? "台積電" : "NVIDIA"}
                        required className="!text-xs !py-2" />
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <label className="block section-label mb-1">購買總額</label>
                      <input type="number" value={hTotalCost} onChange={(e) => handleTotalCostChange(e.target.value)}
                        placeholder="100000" className="!text-xs !py-2" step="any" />
                    </div>
                    <div>
                      <label className="block section-label mb-1">單價/均價</label>
                      <input type="number" value={hAvgCost} onChange={(e) => handleAvgCostChange(e.target.value)}
                        placeholder="850" required className="!text-xs !py-2" step="any" />
                    </div>
                    <div>
                      <label className="block section-label mb-1">數量（自動）</label>
                      <input type="number" value={hQuantity} onChange={(e) => handleQuantityChange(e.target.value)}
                        placeholder="117" required className="!text-xs !py-2" step="any" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block section-label mb-1">現在股價</label>
                      <input type="number" value={hCurrentPrice} onChange={(e) => setHCurrentPrice(e.target.value)}
                        placeholder="920" required className="!text-xs !py-2" step="any" />
                    </div>
                    <div>
                      <label className="block section-label mb-1">購買日期</label>
                      <input type="date" value={hPurchaseDate} onChange={(e) => setHPurchaseDate(e.target.value)} className="!text-xs !py-2" />
                    </div>
                  </div>

                  {/* P&L preview */}
                  {hQuantity && hAvgCost && hCurrentPrice && (() => {
                    const qty = parseFloat(hQuantity), cost = parseFloat(hAvgCost), cur = parseFloat(hCurrentPrice);
                    if (isNaN(qty) || isNaN(cost) || isNaN(cur)) return null;
                    const pnl = (cur - cost) * qty;
                    const pct = cost > 0 ? ((cur - cost) / cost * 100) : 0;
                    return (
                      <div className="bg-white rounded-lg p-2 text-center">
                        <p className={`text-xs font-semibold ${pnl >= 0 ? "text-[var(--green)]" : "text-[var(--red)]"}`}>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {h.purchase_date && (
                              <span className="text-[10px] text-[var(--text-muted)] bg-[var(--border)] rounded-full px-2 py-0.5">
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-[var(--text-muted)]">
                            ...
                            <span className="flex items-center gap-1">現價{" "}
                              <input type="number" defaultValue={h.current_price}
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          <button onClick={() => handleDeleteHolding(h.id)}
                            className="opacity-0 group-hover:opacity-100 text-xs p-1 hover:text-[var(--red)] transition-opacity text-[var(--text-muted)]">🗑️</button>
                        </div>
                      </div>
                    </div>
                  );
      ...
      {investAccounts.length === 0 && !showAccountForm && (
        <div className="card text-center py-8">
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1">點右上角「+ 新增帳戶」開始追蹤</p>
        </div>
      )}
    </div>
  );
